...
export default function ListAdminPage(props: Props) {
  ...
  const memorizedCompoment = useMemo(() => {
    ...
    const filteredList = search.length === 0 ? props.list : props.list.filter((item) => item.title.toLowerCase().includes(search.toLowerCase()) || (item.description && item.description.toLowerCase().includes(search.toLowerCase())));
    if (filteredList.length === 0) return <h2>{props.emptyListText}</h2>;

    return filteredList.map((item) => (
      <li key={item.id}>
        {item.image && <Image src={item.image} width={150} height={100} alt={item.altImage ?? ""} placeholder="blur" blurDataURL={item.blurImage} />}
        ...
        <div className={styles.iconButtonsArea}>
          {props.listButtons.map((button, index) => (
            <IconButton key={index.toString()} icon={button.icon(item.id)} size={32} onClick={() => handleButtonClick(item.id, button)} />
          ))}
        </div>
      </li>
    ));
  }, [props.list, props.emptyListText, props.listButtons, search, handleButtonClick]);
  ...
  return (
    <div className={styles.container} ref={containerRef}>
      <div className={styles.containerInside}>
        ...
        <Form contextLoader={formContext.contextLoader} submit={({ searchQuery }: { searchQuery: string }) => setSearch(searchQuery)} className={styles.form}>
          <Input type="text" placeholder="Pesquisar" name="searchQuery" id="searchQuery" customComponent={({ ref, ...props }) => <CustomInput {...props} />} />
