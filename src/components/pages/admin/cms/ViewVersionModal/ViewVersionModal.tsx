...
export default function ViewVersionModal(props: Props) {
  ...
  const loadVersionData = useCallback(async () => {
    ...
    try {
      const version = await cmsClient.getVersion(props.pageId, props.versionId);
  ...
  const copyToClipboard = useCallback(() => {
    if (versionData) {
      navigator.clipboard
        .writeText(JSON.stringify(versionData, null, 2))
        .then(() => toast.success("JSON copiado para a área de transferência"))
        .catch(() => toast.error("Erro ao copiar JSON"));
    }
  }, [versionData]);
  ...
  return (
    <div
      className={[styles.backdrop, props.visible ? "" : styles.hidden].join(
        " ",
      )}
    >
      <div className={styles.modal}>
        ...
        {loading && (
          <div className={styles.loading}>Carregando dados da versão...</div>
        )}
