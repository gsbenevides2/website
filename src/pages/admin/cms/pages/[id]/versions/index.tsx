...
const ViewVersionModal = dynamic(
  () =>
    import("@/components/pages/admin/cms/ViewVersionModal/ViewVersionModal"),
  {
    ssr: false,
  },
);
...
export default function Versions() {
  ...
  const loadVersions = useCallback(async () => {
    ...
    setVersions(
      versions.docs.map((version) => ({
        ...
        description:
          version.id === pageData?.latestVersion
            ? "Versão atual"
            : "Versão anterior",
      })),
    );
  ...
  const setLatestVersion = useCallback(
    ...
    <>
      ...
      <ListAdminPage
        ...
        seoProps={{
          ...
          title: "Administração das Versões do Guilherme",
          description:
            "Gerencie as versões das páginas do site do Guilherme. Crie, edite, exclua e altere a visibilidade das versões.",
          keywords: [
            "administração",
            "versões",
            "páginas",
          ...
          image: getOpenMediaImageForNextSeo(
            "Administração das Versões do Guilherme",
          ),
          noFollow: true,
          canonical:
